...
code_golf_dir = "/Users/mingzhe/Projects/Free-Solo/kaggle/input/google-code-golf-2025/"
...
def verify_program_full(task_num, code_path, examples):
    task_name, task_path = "task_with_imports", code_path
    spec = importlib.util.spec_from_file_location(task_name, task_path)
    ...
    def verify(example_subset):
        ...
        if error:
            print(f"Error: {error}")
        return right, wrong, expected, error

    arc_agi_right, arc_agi_wrong, arc_agi_expected, arc_agi_error = verify(examples["train"] + examples["test"])
    arc_gen_right, arc_gen_wrong, arc_gen_expected, arc_gen_error = verify(examples["arc-gen"])
    
    if arc_agi_wrong + arc_gen_wrong == 0:
        return True, (arc_agi_right, arc_agi_wrong, arc_agi_expected), (arc_gen_right, arc_gen_wrong, arc_gen_expected, arc_gen_error)
    else:
        return False, (arc_agi_right, arc_agi_wrong, arc_agi_expected), (arc_gen_right, arc_gen_wrong, arc_gen_expected, arc_gen_error)
